import * as csStorage from "./storage.js";
import * as csSupabase from "./supabase.js";
import { buildAboutHtml } from "./ui.js";

/* --- Safety net: never let a single runtime error blank the app --- */
(function () {
  function rqShowFatal(msg) {
    try {
      ...
      if (el) {
        ...
        // Last resort: alert only if absolutely nothing else exists.
  ...
  window.addEventListener("error", function (ev) {
    ...
    rqShowFatal("CineSafari hit an error. If this happened after an update, open the site once with ?nosw=1 to clear the cache, then reload.");
  });
    ...
    render: function (body, close) {
      ...
      function applyTheme() {
        if (state.theme === "light") {
          ...
          if (els.themeToggle) els.themeToggle.textContent = "☀︎";
        } else {
...
async function loadBecauseYouLiked(tmdbId, seedTitle, mediaType) {
  ...
  els.message.textContent = "Loading more like this…";
  ...
  try {
    ...
    if (!state.discoverResults.length) {
      els.message.textContent = "No matches found — try a different mood or broaden your filters.";
    } else {
    ...
    els.message.textContent = "We couldn’t load similar titles. Please try again.";
  } finally {
    render();
  }
}
...
async function loadMoodForDiscover() {
  ...
  els.message.style.display = "block";
  els.message.textContent = "Loading " + MOODS[moodKey].label.toLowerCase() + " picks…";
  ...
  try {
    ...
    if (cached && Array.isArray(cached.results) && cached.results.length) {
      ...
      els.message.style.display = "block";
      els.message.textContent = "Offline — showing the last saved Discover results.";
    } else {
      state.discoverResults = [];
      els.message.textContent = "We couldn’t load Discover picks. Please check your connection.";
    }
  } finally {
    render();
  }
  ...
  if (!els.message || !els.grid) return;
  els.message.style.display = "block";
  els.message.textContent = state.includeTv ? "Loading popular titles…" : "Loading popular films…";
  ...
  try {
    ...
    console.error(err);
    state.discoverResults = [];
    els.message.textContent = "We couldn’t load popular titles. Please check your connection.";
  } finally {
    render();
  }
}
  ...
  els.message.style.display = "block";
  els.message.textContent = state.includeTv ? "Loading upcoming and on-air titles…" : "Loading upcoming releases…";
  renderSkeletonGrid(12);
  ...
  try {
    ...
    if (cached && Array.isArray(cached.results) && cached.results.length) {
      ...
      if (els && els.message) {
        els.message.style.display = "block";
        els.message.textContent = "Offline — showing the last saved Radar results.";
      }
      render();
      return;
    }
    console.error(err);
    state.radarResults = [];
    els.message.textContent = "We couldn’t load Radar titles. Please check your connection.";
  } finally {
    render();
  }
}
...
async function loadForYouFallbackPopular() {
  ...
  try {
    ...
    render();
      ...
      async function performDiscoverSearch() {
        ...
        els.message.style.display = "block";
        els.message.textContent = "Searching TMDB…";
        renderSkeletonGrid(10);
        try {
  ...
  els.message.style.display = "block";
  els.message.textContent =
    "Fetching recommendations based on your favourite genres…";
  els.grid.innerHTML = "";
  ...
  try {
    ...
    if (cached && Array.isArray(cached.results) && cached.results.length) {
      ...
      state.forYouLoaded = true;
      els.message.style.display = "block";
      els.message.textContent = "Offline — showing your last saved recommendations.";
      state.forYouLoading = false;
      render();
      return;
    }
    ...
    state.forYouLoaded = false;
    els.message.textContent =
      "We couldn’t load your recommendations. Please check your connection.";
  } finally {
    state.forYouLoading = false;
    render();
  }
...
async function refreshSmartList(listId) {
  ...
  if (!list || list.type !== "smart") return;
  els.message.style.display = "block";
  els.message.textContent = "Refreshing list…";
  try {
...
function render() {
        ...
        if (state.localMoodLoading) {
          els.message.style.display = "block";
          els.message.textContent = "Fetching genre info so Mood filtering works on your saved lists…";
        } else if (state.streamingLoading && streamingPref() !== "any") {
          els.message.style.display = "block";
          els.message.textContent = "Checking streaming availability for this list…";
...
try { rqCloudSyncOnAuthChange(); } catch (e) {}render();
